/*
    ...
    application-level constraints limit these values to be much smaller than they are expressed
    here.
*/
...
/*
    We have to disable foreign key checks so that we can drop the tables; this will only disable it
...
CREATE TABLE users (
    ...
    */
    ...
    /* All authentication is through Steam, so we don't need to store a password for the user. */
    /* steam_id must be a BIGINT, as they are 17 digits long. */
    steam_id             BIGINT        NOT NULL  UNIQUE,

    /* MariaDB is case insensitive by default, which is what we want. */
    ...
    /* 0 is a normal user, 1 is a staff, 2 is a full administrator */
    admin                INT           NOT NULL  DEFAULT 0,

    /* Used to show who is a legitimate player on the leaderboard. */
    ...
    */
    ...
    /* The amount changed in the last race (can be positive or negative). */
    ...
    */
    ...
    /* The amount changed in the last race (can be positive or negative). */
    unseeded_trueskill_change  FLOAT      NOT NULL  DEFAULT 0,
    ...
    */
    ...
    /* Currently used to store the next build */
    ranked_solo_metadata          INT           NULL      DEFAULT NULL,
    ...
    */
    ...
    /* The amount changed in the last race (can be positive or negative) */
    diversity_trueskill_change  FLOAT      NOT NULL  DEFAULT 0,
...
CREATE TABLE races (
    ...
    */
    ...
    /* Valid characters are contained in the "raceValidateRuleset()" function. */
    /* (You can't name columns "character" in MariaDB, so we will use the Lua name for this instead.) */
    player_type     VARCHAR(50)  NULL,

    /* Valid entries are contained in the "raceValidateRuleset()" function. */
    goal            VARCHAR(50)  NULL,

    /* Valid entries are contained in the "raceValidateRuleset()" function. */
    difficulty      VARCHAR(50)  NULL  DEFAULT "normal",
