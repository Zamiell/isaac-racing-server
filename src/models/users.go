...
func (*Users) Login(steamID string) (int, string, int, string, error) {
	...
	err := db.QueryRow(`
		SELECT id, username, admin, stream_url
		FROM users
		WHERE steam_id = ?
	`, steamID).Scan(&userID, &username, &admin, &streamURL)
	if err == sql.ErrNoRows {
		return 0, "", 0, "", nil
	} else if err != nil {
		return 0, "", 0, "", err
	} else {
		return userID, username, admin, streamURL, nil
	}
}
...
func (*Users) FindUserFromStreamURL(streamURL string) (int, string, error) {
	// Get the user ID that matches a stream URL
	var userID int
	var username string
	err := db.QueryRow(`
		SELECT userID, username
		FROM users
		WHERE stream_url = ? COLLATE NOCASE
	`, streamURL).Scan(&userID, &username)
	if err == sql.ErrNoRows {
		return 0, "", nil
	} else if err != nil {
		return 0, "", err
